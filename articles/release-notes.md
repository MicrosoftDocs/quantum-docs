...
- You can now estimate the price of your job in azure-quantum package by using estimate_cost (link needed) for the target that you are using with the Qiskit, Cirq and pass-through feature.
- The azure-quantum package now supports submitting Qiskit lists of length 1. 
- Fixed issue#198 (link needed) in azure-quantum package where the Qiskit jobs were causing incorrect results in circuits with helper qubits.
...
- Fixed an issue in azure-quantum package so errors returned by the Azure Quantum service when using the Qiskit or Cirq feature are now propagated up. 
- Fixed an [issue](https://github.com/microsoft/qsharp-compiler/issues/1297) where IntelliSense information was not updated when the ExecutionTarget property was modified in the project file 
