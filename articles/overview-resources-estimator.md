...
> [!NOTE]
> If you run into issues when you work with the Resource Estimator, check out the [Troubleshooting page](xref:microsoft.quantum.azure.common-issues#azure-quantum-resource-estimator).
...
> [!NOTE]
> Unless you specify a different value, the default value for the qubit model is `"qubit_gate_ns_e3"`.
...
You can customize predefined qubit parameters by specifying the name and then updating any of the other values. For example, to decrease the error rate of two-qubit joint measurements in `qubit_maj_ns_e4`, write the following code:
