...
1. For Qiskit version 1 and version 2 support, install the `qiskit` extra:
   ...
   > [!IMPORTANT]
   > If you're updating from a previous Qiskit environment, then see [Update the azure-quantum package with Qiskit support](xref:microsoft.quantum.update-qdk#update-the-qdk-azure-package-with-qiskit-support-in-a-virtual-python-environment-recommended).
