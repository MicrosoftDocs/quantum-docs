...
YOu may first attempt to define $O$ so that $O\ket{x} = \ket{f(x)}$, but this has a couple of problems.
...
Now $O = O^\dagger$ by construction and you have resolved both of the earlier problems.
...
In the case of defining the oracle $O$, you can similarly use that any state $\ket{\psi}$ on $n + m$ qubits can be written as
...
Choosing the best way to implement an oracle depends heavily on how this oracle is be used within a given algorithm.
...
For more details, see the discussion in [Gilyén *et al*. 1711.00465](https://arxiv.org/abs/1711.00465).
