...
ms.topic: conceptual
no-loc: [Base QIR, target, targets]
...
IonQ's targets correspond to a **:::no-loc text="Base QIR":::** profile. For more information about this target profile and its limitations, see [Understanding target profile types in Azure Quantum](xref:microsoft.quantum.target-profiles#create-and-run-applications-for-no-control-flow-profile-targets). 
...
- Target Execution Profile: [:::no-loc text="Base QIR":::](xref:microsoft.quantum.target-profiles)
...
IonQ's targets correspond to the [:::no-loc text="Base QIR"::: profile](xref:microsoft.quantum.target-profiles#create-and-run-applications-for-no-control-flow-profile-targets). This profile can't run quantum operations that require the use of the results from qubit measurements to control the program flow. 
