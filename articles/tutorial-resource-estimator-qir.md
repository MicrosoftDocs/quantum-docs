...
The Azure Quantum Resource Estimator is build upon [Quantum Intermediate Representation (QIR)](xref:microsoft.quantum.concepts.qir), the forward-looking, 
fully interoperable specification for quantum programs. QIR serves as a common interface between quantum programming languages/frameworks and 
targeted quantum computation platforms. Because the Azure Quantum Resource Estimator takes a QIR program as input, it supports any language that translates to QIR. For example, it can be used by popular quantum SDKs and languages such as Q# and Qiskit.  

This tutorial shows how to write and submit QIR to the [Azure Quantum Resource Estimation](xref:microsoft.quantum.overview.resources-estimator) target. This tutorial uses [PyQIR](https://github.com/qir-alliance/pyqir) to generate QIR, however, you can use any other source of QIR.
...
> * Submit a QIR job to the Azure Quantum Resource Estimator
...
First, you import some Python classes and functions from `azure.quantum`, `qiskit`, and `pyqir`.  You won't use Qiskit to build quantum circuits directly, but you'll use `AzureQuantumJob` and `job_monitor`, which are built on top of the Qiskit ecosystem.
...
location, which you can obtain from the **Overview** page in your Azure Quantum workspace in the [Azure portal](https://portal.azure.com).
...
```
## Define a function to create resource estimation job from QIR
...
You define your program, set a target, and submit your job for computation. To use the Azure Quantum Resource Estimator, the target is the `microsoft.estimator`.  When you set the target, and you can provide optional parameters such as the qubit type, one qubit gate time, etc.
The Azure Quantum Resource Estimator will return detailed output that can be visually rendered or programmatically parsed.

When submitting a resource estimate request for your program, you can optionally specify some characteristics. There are three top-level input parameters that can be customized: `errorBudget`, 
which is the overall allowed error, `qecScheme`, which is the quantum error correction (QEC) scheme, and `qubitParams`, which are the physical qubit parameters. For more 
information about the pre-defined input parameters, see [Input parameters of the Azure Quantum Resource Estimator](xref:microsoft.quantum.overview.resources-estimator#input-parameters).

Let's implement a generic function that takes as input the Azure Quantum Resource Estimator provider and the QIR bitcode of the quantum program.  It returns as result an Azure Quantum job. 
The Azure Quantum Resource Estimation input parameters can be passed via keyword arguments to the function.
...
def resource_estimation_job_from_qir(provider: AzureQuantumProvider, bitcode: bytes, **kwargs):
    ...
    # Finally, we are creating the Azure Quantum jon object and return it
...
Let's now create some QIR bitcode using PyQIR generator.  Here, we build a controlled S gate using three T gates and two CNOT gates.
...
You can use the function you defined above together with the `bitcode()` function from PyQIR to generate a resource estimation job. You can also pass resource estimation specific arguments, for example, setting the error rate to 5%.
