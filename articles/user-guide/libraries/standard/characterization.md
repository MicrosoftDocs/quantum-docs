...
You proceed as in the quantum case by using phase kickback to turn applications of a black-box operation into rotations by an unknown angle, but measure the auxiliary qubit that you rotate at each step immediately following the rotation.
This has the advantage that you only require a single additional qubit to perform the phase kickback described in the quantum case, as you then learn the phase from the measurement results at each step in an iterative fashion.  
...
Here, this is emphasized by briefly describing iterative phase estimation at a theoretical level in the Bayesian parameter estimation formalism before proceeding to describe the statistical algorithms provided in the Q# canon for solving this classical inference problem.
...
\end{align}
As this process is iterated over multiple `Result` values, eigenstates that do not have maximal values of $\prod_k\Pr(\text{Result}\_k|\phi\_j)$ are exponentially suppressed.
As a result, the inference process tends to converge to states with a single eigenvalue if the experiments are chosen properly.
...
Following traditional classical terminology, $\eqref{eq:phase-est-likelihood}$ is called the *likelihood function* for iterative phase estimation.

Having observed a `Result` from the iterative phase estimation likelihood function, you can then use Bayes' rule to prescribe what you should believe the phase to be following that observation.
Concretely,
\begin{equation}
...
such as Shor's algorithm, cannot use exact Bayesian inference within its phase estimation step.  For this reason, Q# also provides implementations
...
$$
Moreover, if $U$ is a simulation of a dynamical generator, as is the case for [Hamiltonian simulation](xref:microsoft.quantum.libraries.overview.applications#hamiltonian-simulation), you interpret $\phi$ as an energy.
Thus, using phase estimation with continuous queries allows you to learn the simulated [energy spectrum of molecules](https://arxiv.org/abs/quant-ph/0604193), [materials](https://arxiv.org/abs/1510.03859) or [field theories](https://arxiv.org/abs/1111.3633v2) without having to compromise our choice of experiments by requiring $t$ to be an integer.
...
This Gaussian assumption allows you to use an analytical formula for the experiment that minimizes the posterior variance.
...
The ability to step backwards also allows the algorithm to learn even if the initial prior standard deviation is inappropriately small.
