...
In this article, you learn how to visually represent quantum algorithms with quantum circuit diagrams in Visual Studio Code (VS Code) or Jupyter Notebook.

For more information about quantum circuit diagram, see [Quantum circuits conventions](xref:microsoft.quantum.concepts.circuits).
...
- The latest Azure Quantum `qdk` Python library with the optional `azure` extra.
...
- The latest Azure Quantum `qdk` Python library with the optional `jupyter` extra, and the `ipykernel` package.  
...
To visualize the quantum circuit for a single Q# operation, choose on the **Circuit** code lens above the operation declaration.
...
1. In the **Run and Debug** pane, expand the **Quantum Circuit** dropdown in the **VARIABLES** menu to show the circuit as you step through the program.
...
1. The current quantum circuit is shown in the **QDK Circuit** panel. This circuit diagram represents the current state of the simulator, which is the gates that have been applied to up until the current point in your program.
...
In Jupyter Notebook, you can visualize quantum circuits with the `qsharp` and `widgets` packages. The `widgets` package provides a widget that renders a quantum circuit diagram as an SVG image.
...
The downside of traced circuits is that they only capture the measurement outcome and the consequent gate applications for a single simulation. In the above example, if the measurement outcome is `Zero`, you don't see the `X` gate in the diagram. Another run of the simulation might show a slightly different circuit.

### Target profile

The QIR target profile that you select affect the generation of circuit diagrams. Target profiles are used to specify the capabilities of the target hardware, and the restrictions that are imposed on the quantum program.
...
1. When target profile is set to **Unrestricted**, **Adaptive RI**, or **Adaptive RIF**, the gates in the circuit diagram correspond exactly to the quantum operations that are invoked in the Q# program.
...
1. When the target profile is **Base**, the circuit looks different. Because Base profile targets don't allow qubit reuse after measurement, the measurement is now performed on an entangled qubit instead. The `Reset` operation isn't a supported gate in base profile, so the operation is dropped. The resulting circuit matches what would be run on hardware if this program is submitted to Azure Quantum with this target profile.
