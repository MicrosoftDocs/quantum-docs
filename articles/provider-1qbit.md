...
******
Please note that all parameter values are required to be of JSON string type.  The types listed below are the types the solvers expect within the string value.  For example, 'frac_icm_thermal_layers' is listed as type double, so it is expected to be given in the format:
